function [phaseOut,omegaOut] = fireflySimulation3_Sich(varargin)
...
for i = 2:SimulationLength
    ...
    for j=1:n
        if phi(i-1,j) < 1
            ...
            % fprintf('%d %d %f %d \n',i,j,phi(i,j),f(j))
            in=in+1;
            interactions(in,[1:3,11]) = [2 i j f(j)];
         ...
         if phi(i,j) >= 1        % if node j is at maximum
           ...
           % f(j)=1;             % fire on every peak
            ...
            if f(j) == 1 && fire == 0       % if node j fires and firing has not already occured at this time.
                fire = 1; % set flag for fire event at time i % -> node fired -> for next node -> stoppt bei Zeitpunkt i -> schaut ob ein Knoten feuert
                ...
                for k = setdiff(randperm(n),j,'stable') % k = all nodes but j
                    if phi(i,k) > phiref(k) % if the phase of node k is above phiref
                       % PHASE COUPLING FUNCTION
                        ...
                        in=in+1;
                        interactions(in,1:9) = [1 i j k 1 filterType(E(length(E(:,k))-FilterLength+1:length(E(:,k)),k)) phi(i,k)-phi(i-1,k) Beta*sin(2*pi*phi(i,k)) domega(k,1)]; 
                    else
                        in=in+1;
                        interactions(in,1:6) = [1 i j k 0 filterType(E(length(E(:,k))-FilterLength+1:length(E(:,k)),k))]; %col 5 == 0 means fire within tref
                    end
                end
            end
...
%                 end
                in=in+1;
                interactions(in,1:10) = [-2 i j j 1 filterType(E(length(E(:,j))-FilterLength+1:length(E(:,j)),j)) -1 0 0 omega(j)]; 
            end
%            [j;E(:,j)]
            if sum(E(length(E(:,j))-FilterLength+1:length(E(:,j)),j)) == 0 && length(interactions(interactions(:,3)==j & interactions(:,11)==1,1)) > 8
                ...
                in=in+1;
                interactions(in,1:10) = [-3 i j j 1 filterType(E(length(E(:,j))-FilterLength+1:length(E(:,j)),j)) -1 0 0 omega(j)]; 
            end
        end
    end
